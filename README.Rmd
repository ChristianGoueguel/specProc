...
```{r include=FALSE}
bc_spec <- arrow::read_parquet("~/Documents/Laserag/R&D projects/Soil/Eurofins/SOC/processed data/bc_spec")
```

```{r include=FALSE}
carbonLine <- bc_spec %>%
  select(spectra_id, calibration, `247.700233`:`248.077678`) %>%
...
carbonLine %>%
  ...
  geom_line(aes(colour = spectra_id, group = spectra_id)) +
