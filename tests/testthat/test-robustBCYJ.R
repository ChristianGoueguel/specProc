...
test_that("robustBCYJ transforms the numeric variables", {
  res <- robustBCYJ(test_data)
  ...
  sub <- robustBCYJ(test_data, var = c("gamma", "normal"), type = "YJ")
  expect_equal(sub$summary$variable, c("gamma", "normal"))
  expect_true(all(sub$summary$method == "YJ"))
})

test_that("robustBCYJ validates its inputs", {
  expect_error(robustBCYJ(), "Missing 'data' argument.")
  expect_error(robustBCYJ(as.matrix(test_data[1:3])), "Input 'data' must be a data frame or tibble.")
  expect_error(robustBCYJ(test_data, var = 1), "The 'var' argument must be a character vector or NULL.")
  expect_error(robustBCYJ(test_data, var = "nope"), "not present")
