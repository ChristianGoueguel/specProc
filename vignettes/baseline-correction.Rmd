...
```{r setup}
source("~/Documents/Projects/Packages/specProc/R/baselineRemoval.R")
source("~/Documents/Projects/Packages/specProc/R/plotSpec.R")
```
...
function `baselineRemoval` to perform such a task.

```{r message=FALSE, warning=FALSE}
baseline_fit <- spec_tbl %>%
  select(`390.03027`:`500.03397`) %>%
  baselineRemoval(degree = 7)
```

As one can see, the `baselineRemoval` function provides a list containing two
