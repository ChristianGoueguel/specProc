...
(medcouple-adjusted fences) and `directional_outlyingness()` provide the
same screening for a single vector.

### 4. Averaging replicates

After screening, reduce each sample to one spectrum. `average()`
...
in the 393–397 nm window. `multipeak_fit()` fits them jointly as a sum
of exact Voigt profiles on a common baseline. Each line’s center,
Gaussian and Lorentzian widths (FWHM) and area are estimated by
...
<img src="man/figures/README-multipeak-plot-1.png" width="90%" />
...
The exact Voigt profile is also available directly. `voigt_profile()`
uses the Faddeeva function and is accurate to about 1e-10. The faster
`pseudo_voigt_profile()` approximation deviates from it by up to about
1%:
...
| Operation                       | Time   |
|---------------------------------|--------|
| `baseline_arpls()`, 400 spectra | 0.6 s  |
| `baseline_als()`, 400 spectra   | 0.5 s  |
| `epo(ncomp = 3)`                | 0.4 s  |
| `snv()`                         | 0.08 s |
| `voigt_profile()`, 10⁶ points   | 0.07 s |
